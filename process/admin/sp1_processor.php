...
if ($method == 'fetch_sp1') {
    ...
    // $sp1 = "SELECT * FROM ialert_audit WHERE falp_group = 'Secondary 1 Process' AND section = '' AND edit_count != 0 AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND provider = 'fas' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL ";
    $sp1 = "SELECT * FROM ialert_audit where falp_group = 'Secondary 1 Process'";
    if (!empty($section)) {
    ...
    $sp1 .= " AND edit_count = 0 AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%' AND provider = 'FAS' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')";
    $stmt = $conn->prepare($sp1);
    $stmt->execute();
    if ($stmt->rowCount() > 0) {
...
if ($method == 'count_sp1') {
    ...
    foreach ($stmt->fetchALL() as $x) {
        ...
        foreach ($stmt2->fetchALL() as $j) {
            ...
            $count_pending = "SELECT (COUNT(*) + $total_ir_pending) as totals FROM ialert_audit WHERE edit_count != 0 AND section IN ('sp1section1','sp1section2','sp1section3','sp1section4','sp1section5','sp1section6','sp1section7','sp1section8','sp1section9') AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL";
...
if ($method == 'total_sp1') {
    $dateFrom = $_POST['dateFrom'];
    $dateTo = $_POST['dateTo'];
    $count_total = "SELECT count(*) as grand_total9 from ialert_audit where section IN ('sp1section1','sp1section2','sp1section3','sp1section4','sp1section5','sp1section6','sp1section7','sp1section8','sp1section9') AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'fas'";
