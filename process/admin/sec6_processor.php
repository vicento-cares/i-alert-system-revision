...
if ($method == 'fetch_sec6') {
        ...
        // $sec6 = "SELECT * FROM ialert_audit WHERE section = 'Section6' AND edit_count != 0 AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND provider = 'fas' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL";
         $sec6 ="SELECT * FROM ialert_audit where section = 'Section6' AND edit_count != 0 AND provider = 'FAS' AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%'
            UNION ALL
         SELECT * FROM ialert_audit where pd = 'IR' AND section = 'Section6' AND edit_count = 0 AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%' AND provider = 'FAS'";
...
if ($method == 'count_section6') {
	...
	foreach($stmt->fetchALL() as $x){
        ...
        $days_notif = date("Y-m-d", strtotime('+4 day',strtotime($date_audited)));
            ...
            $count_na = "SELECT COUNT(*) as total FROM ialert_audit WHERE pd = 'IR' AND section = 'Section6' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS' AND audit_type LIKE '$audit_type%' AND edit_count = 0 AND date_recieved IS NULL";
        $stmt2 = $conn->prepare($count_na);
        $stmt2->execute();
        foreach($stmt2->fetchALL() as $j){
            $total_ir_pending = $j['total'];
            $count_pending = "SELECT (COUNT(*) + $total_ir_pending) as totals FROM ialert_audit WHERE edit_count != 0 AND section = 'Section6' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL";
...
if ($method == 'total_sec6') {
	$dateFrom = $_POST['dateFrom'];
	$dateTo = $_POST['dateTo'];
	$count_total = "SELECT count(*) as grand_total6 from ialert_audit where section = 'Section6' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'fas'";
