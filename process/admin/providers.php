...
if ($method == 'fetch_provider') {
    ...
    $sec1 = "SELECT * FROM ialert_audit WHERE edit_count != 0 AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND provider = '$provider' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')";
...
if ($method == 'count_provider') {
    ...
    foreach ($stmt->fetchALL() as $x) {
        ...
        $count_na = "SELECT COUNT(*) as total FROM ialert_audit WHERE edit_count !=0 AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = '$provider' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL";
...
if ($method == 'total_provider') {
    $dateFrom = $_POST['dateFrom'];
    $dateTo = $_POST['dateTo'];
    $provider = $_POST['provider'];
    $count_total = "SELECT count(*) as grand_total from ialert_audit where (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = '$provider'";
