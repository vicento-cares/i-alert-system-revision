...
if ($method == 'fetch_provider') {
        ...
        $sec1 = "SELECT * FROM ialert_audit WHERE edit_count != 0 AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND provider = '$provider' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL";
