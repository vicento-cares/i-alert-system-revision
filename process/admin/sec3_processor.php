...
if ($method == 'fetch_sec3') {
    ...
    $sec3 = "SELECT * FROM ialert_audit where section = 'Section 3' AND edit_count != 0 AND provider = 'fas' AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')
            UNION ALL
         SELECT * FROM ialert_audit where pd = 'Written IR' AND section = 'Section 3' AND edit_count = 0 AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%' AND provider = 'fas' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')";
...
if ($method == 'total_sec3') {
    $dateFrom = $_POST['dateFrom'];
    $dateTo = $_POST['dateTo'];
    $count_total = "SELECT count(*) as grand_total3 from ialert_audit where section = 'Section 3' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'fas'";
...
if ($method == 'count_section3') {
    ...
    foreach ($stmt->fetchALL() as $x) {
        ...
        $count_na = "SELECT COUNT(*) as total FROM ialert_audit WHERE pd = 'Written IR' AND section = 'Section 3' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'fas' AND audit_type LIKE '$audit_type%' AND edit_count = 0 AND date_recieved IS NULL";
        $stmt2 = $conn->prepare($count_na);
        $stmt2->execute();
        foreach ($stmt2->fetchALL() as $j) {
            $total_ir_pending = $j['total'];
            $count_pending = "SELECT (COUNT(*) + $total_ir_pending) as totals FROM ialert_audit WHERE edit_count != 0 AND section = 'Section 3' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'fas' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL";
