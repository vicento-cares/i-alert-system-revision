...
if ($method == 'fetch_sec3') {
	...
	$sec3 = "SELECT * FROM ialert_audit where section = 'Section3' AND edit_count != 0 AND provider = 'FAS' AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')
            UNION ALL
         SELECT * FROM ialert_audit where pd = 'Written IR' AND section = 'Section3' AND edit_count = 0 AND date_recieved IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND audit_type LIKE '$audit_type%' AND provider = 'FAS' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo')";
...
if ($method == 'count_section3') {
	...
	foreach ($stmt->fetchALL() as $x) {
		...
		$count_na = "SELECT COUNT(*) as total FROM ialert_audit WHERE pd = 'Written IR' AND section = 'Section3' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS' AND audit_type LIKE '$audit_type%' AND edit_count = 0 AND date_recieved IS NULL";
