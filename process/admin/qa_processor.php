...
if ($method == 'fetch_qa') {
	...
	// $qa = "SELECT * FROM ialert_audit WHERE falp_group = 'QA' AND section = '' AND edit_count != 0 AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND provider = 'fas' AND audit_type LIKE '$audit_type%' AND date_recieved IS NULL ";
	$qa = "SELECT * FROM ialert_audit where falp_group = 'QA'";
	if (!empty($section)) {
	...
	UNION ALL
	SELECT * FROM ialert_audit where pd = 'IR' AND falp_group = 'QA'";
	if (!empty($section)) {
		$qa .= " AND section = '$section'";
	}
...
if ($method == 'count_qa') {
	...
	foreach ($stmt->fetchALL() as $x) {
		...
		$count_na = "SELECT COUNT(*) as total FROM ialert_audit WHERE pd = 'IR' AND section IN ('qa-initial','qa-final') AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS' AND audit_type LIKE '$audit_type%' AND edit_count = 0 AND date_recieved IS NULL";
...
if ($method == 'total_qa') {
	$dateFrom = $_POST['dateFrom'];
	$dateTo = $_POST['dateTo'];
	$count_total = "SELECT count(*) as grand_total9 from ialert_audit where section IN ('qa-initial','qa-final') AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'fas'";
